...
class InterBagAttention(BagRE):
    """
    Instance attention for bag-level relation extraction.
    """

    def __init__(self, bag_encoder, num_class, rel2id):
        """
        Args:
        ...
        self.fc = nn.Linear(self.bag_encoder.hidden_size, num_class)
    ...
    def infer(self, bag):
        ...
        masks = []
        for item in bag:
            token, pos1, pos2, mask = self.bag_encoder.tokenize(item)
            tokens.append(token)
            pos1s.append(pos1)
            pos2s.append(pos2)
            masks.append(mask)
    ...
    def forward(self, label, scope, token, pos1, pos2, mask=None, train=True, bag_size=0):
        ...
        if mask is not None:
            rep = self.bag_encoder(token, pos1, pos2, mask) # (nsum, H) 
        else:
            rep = self.bag_encoder(token, pos1, pos2) # (nsum, H) 
