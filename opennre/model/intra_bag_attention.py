...
class IntraBagAttention(BagRE):
    ...
    def forward(self, label, scope, token, pos1, pos2, mask=None, train=True, bag_size=0):
        ...
        Return:
            bag_rep, (B, 3H), representation of bag
        """
        if bag_size > 0:
            token = token.reshape(-1, token.size(-1))
            pos1 = pos1.reshape(-1, pos1.size(-1))
            pos2 = pos2.reshape(-1, pos2.size(-1))
            if mask is not None:
                mask = mask.reshape(-1, mask.size(-1))
        else:
            begin, end = scope[0][0], scope[-1][1]
            token = token[:, begin:end, :].reshape(-1, token.size(-1))
            pos1 = pos1[:, begin:end, :].reshape(-1, pos1.size(-1))
            pos2 = pos2[:, begin:end, :].reshape(-1, pos2.size(-1))
            if mask is not None:
                mask = mask[:, begin:end, :].reshape(-1, mask.size(-1))
        ...
        if train:
            if bag_size == 0:
                ...
                rep = rep.reshape(batch_size, bag_size, -1)
            ...
            if bag_size == 0:
                ...
                att_score = torch.matmul(rep, self.fc.weight.data.transpose(0, 1)) # (nsum, H) * (H, N) -> (nsum, N)
                att_score = att_score.reshape(batch_size, bag_size, -1) # (B, bag, N)
                rep = rep.reshape(batch_size, bag_size, -1) # (B, bag, H)
